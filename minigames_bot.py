...
TRIGGER_BALANCE = ("баланс", "balance", "бал")
...
def welcome_kb() -> InlineKeyboardMarkup:
    builder = InlineKeyboardBuilder()
    builder.button(text="Хелп", callback_data="quick:help")
    builder.button(text="Баланс", callback_data="quick:balance")
    builder.button(text="Ежедневка", callback_data="quick:daily")
    builder.button(text="VIP", callback_data="quick:buy")
    builder.adjust(2)
    return builder.as_markup()
...
async def cmd_start(message: Message, state: FSMContext):
    ...
    if await is_banned(message.from_user.id):
        ...
        return
    await message.answer(await get_text("welcome"), reply_markup=welcome_kb())


@common_router.message(Command("help"), StateFilter("*"))
@common_router.message(F.text.lower().in_(TRIGGER_HELP), StateFilter("*"))
async def cmd_help(message: Message):
    await message.answer(await get_text("help"))


@common_router.message(Command("balance"), StateFilter("*"))
@common_router.message(F.text.lower().in_(TRIGGER_BALANCE), StateFilter("*"))
...
@common_router.callback_query(F.data == "quick:help")
async def quick_help(call: CallbackQuery):
    await call.message.answer(await get_text("help"))
    await call.answer()


@common_router.callback_query(F.data == "quick:balance")
async def quick_balance(call: CallbackQuery):
    await ensure_user(call.from_user.id, call.from_user.username or "", call.from_user.full_name)
    balance = await get_balance(call.from_user.id)
    await call.message.answer(await get_text("balance", balance=format_amount(balance)))
    await call.answer()
...
@games_router.message(F.text.lower().startswith(TRIGGER_ROULETTE), StateFilter("*"))
...
# =============================================================================
@games_router.message(F.text.lower().startswith(TRIGGER_ROCKET), StateFilter("*"))
...
# =============================================================================
@games_router.message(F.text.lower().startswith(TRIGGER_MINES + " "), StateFilter("*"))
...
# =============================================================================
@games_router.message(F.text.lower().startswith(TRIGGER_MINEGAME + " "), StateFilter("*"))
...
# =============================================================================
@games_router.message(F.text.lower().startswith(TRIGGER_TRANSFER + " "), ~F.reply_to_message, StateFilter("*"))
async def transfer_no_reply(message: Message):
    await message.reply(
        f"Чтобы перевести, ответьте (reply) на сообщение игрока и напишите "
        f"«{TRIGGER_TRANSFER} <сумма>», например: {TRIGGER_TRANSFER} 100кк"
    )


@games_router.message(F.text.lower().startswith(TRIGGER_RPS + " "), ~F.reply_to_message, StateFilter("*"))
async def rps_no_reply(message: Message):
    await message.reply(
        f"Чтобы вызвать на игру, ответьте (reply) на сообщение игрока и напишите "
        f"«{TRIGGER_RPS} <сумма>», например: {TRIGGER_RPS} 100кк"
    )
...
# =============================================================================
async def _claim_daily(user_id: int, username: str, full_name: str) -> str:
    """Общая логика ежедневного бонуса. Возвращает готовый текст ответа."""
    await ensure_user(user_id, username or "", full_name)
    if await is_banned(user_id):
        return await get_text("banned")
    ...
    if last_daily:
        ...
        if remaining.total_seconds() > 0:
            ...
            return await get_text("daily_cooldown", remaining=f"{minutes} мин")
    ...
    return await get_text("daily_bonus", amount=format_amount(amount), cooldown=cooldown_minutes)


@games_router.message(F.text.lower().in_(TRIGGER_DAILY), StateFilter("*"))
async def daily_bonus(message: Message):
    text = await _claim_daily(message.from_user.id, message.from_user.username, message.from_user.full_name)
    await message.answer(text)


@common_router.callback_query(F.data == "quick:daily")
async def quick_daily(call: CallbackQuery):
    text = await _claim_daily(call.from_user.id, call.from_user.username, call.from_user.full_name)
...
@common_router.callback_query(F.data == "quick:buy")
async def quick_buy(call: CallbackQuery, bot: Bot):
    await _offer_vip(call.from_user.id, call.from_user.username, call.from_user.full_name,
...
async def cmd_buy(message: Message, bot: Bot):
    await _offer_vip(message.from_user.id, message.from_user.username, message.from_user.full_name,
                      message.chat.id, bot)


async def _offer_vip(user_id: int, username: str, full_name: str, chat_id: int, bot: Bot):
    await ensure_user(user_id, username or "", full_name)

    if await is_vip(user_id):
        await bot.send_message(chat_id, "У вас уже есть VIP статус.")
        return

    price = await get_setting_int("vip_price_stars")
    daily_mult = await get_setting_float("vip_daily_multiplier")
    await bot.send_message(chat_id, await get_text("vip_offer", price=price, multiplier=daily_mult))
...
async def main():
    ...
    public_commands = [
        ...
        BotCommand(command="buy", description="Купить VIP"),
    ]
    await bot.set_my_commands(public_commands, scope=BotCommandScopeDefault())
    try:
        await bot.set_my_commands(
            public_commands + [BotCommand(command="admin", description="Секретная админ-панель")],
            scope=BotCommandScopeChat(chat_id=OWNER_ID),
        )
    except Exception:
        # если владелец ещё ни разу не писал боту в личку, Telegram может отклонить
        # персональный scope — не критично, командой /admin всё равно можно пользоваться
        pass
