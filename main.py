...
_SQL_ADVANCED = [
    {"title": "1. Транзакции", "content": (
        "Представь перевод денег: списать с одного счёта и зачислить на другой — это "
        "должно произойти ЦЕЛИКОМ или НЕ произойти вообще (иначе деньги могут "
        "«пропасть», если что-то сломается посередине). Для этого нужны транзакции:\n\n"
        "```sql\nBEGIN;\nUPDATE accounts SET balance = balance - 100 WHERE id = 1;\n"
        "UPDATE accounts SET balance = balance + 100 WHERE id = 2;\nCOMMIT;\n```\n\n"
        "`BEGIN` начинает транзакцию, `COMMIT` подтверждает и сохраняет все изменения "
        "разом. Если между ними произошла ошибка — вызывают `ROLLBACK`, который "
        "откатывает ВСЕ изменения транзакции назад, будто их и не было."
    ), "task": "Опиши в комментарии, что случится с балансами, если между двумя UPDATE вызвать ROLLBACK вместо COMMIT."},
    {"title": "2. Мини-проект: аналитика магазина", "content": (
        "Собираем агрегатные функции, JOIN и группировку в один аналитический "
        "запрос — то, чем реально занимаются на работе с базами данных:\n\n"
        "```sql\nSELECT products.category, COUNT(*) AS orders_count, "
        "SUM(orders.price) AS revenue\nFROM orders\nJOIN products ON products.id = "
        "orders.product_id\nWHERE orders.created_at >= '2026-01-01'\n"
        "GROUP BY products.category\nHAVING COUNT(*) > 5\nORDER BY revenue DESC;\n```\n\n"
        "Этот запрос отвечает на реальный бизнес-вопрос: «какие категории товаров "
        "принесли больше всего денег с начала 2026 года, если по ним было больше 5 "
        "заказов». Именно так выглядят настоящие SQL-запросы в аналитике."
    ), "task": "Напиши свой аналитический запрос: выручка по месяцам (сгруппируй по дате) за текущий год."},
]
