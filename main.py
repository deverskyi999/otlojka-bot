...
BOT_FEATURE_SUMMARY = (
    "Команды: /start (приветствие), /support (донаты + поддержка), /admin (админ-панель, только владелец).\n\n"
    ...
    "⏰ Напоминание (парсинг 'через N минут/часов' и 'в ЧЧ:ММ', фоновый воркер шлёт напоминание), "
    "📊 Опрос (нативный Telegram-опрос), 🌍 Перевод текста через ИИ, "
    "🗒️ Заметки (личный список текстовых заметок с удалением), "
    "🎲 Мини-игра (нативные Telegram-дайсы: кубик/дартс/баскетбол/футбол/слоты).\n\n"
    ...
    "статус 'печатает...' пока думает; если в ответе есть блок кода — под сообщением кнопка "
    ...
    "экспорт юзеров в CSV, рассылка всем, модель ИИ по умолчанию (скрыта от юзеров), модель для "
    "картинок, тумблер 'дерзкие ответы на мат', тумблер 'работа в группах' (по упоминанию/ответу), "
    "тумблер 'технические работы', юзернейм поддержки, ключ CryptoBot, доп. инструкции персоне, "
    "премиум-эмодзи на любой текст бота (welcome/ai_reply/hub/support/image/telegraph/reminder/"
    "poll/translate/notes/game — и можно добавить свой ключ), премиум-эмодзи + цвет (Bot API 9.4: "
    "icon_custom_emoji_id/style) + свой текст на каждую пользовательскую кнопку.\n\n"
    "Технически: aiogram 3.20+, SQLite, OpenAI-совместимый чат-эндпоинт прокси imbek.fun, "
    "фоновые asyncio-таски для напоминаний и авто-проверки крипто-донатов."
)
