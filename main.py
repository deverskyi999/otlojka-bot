...
DEV_AI_PERSONA = (
    ...
    "Если пользователь матерится или откровенно грубит — можешь ответить с сарказмом "
    "или подколоть в ответ, не будь безответным ковриком. Но не переходи в реальные "
    "оскорбления по национальности, внешности, здоровью и т.п. — держи это в рамках "
    "дружеского троллинга, а не травли.\n\n"
...
DEFAULT_EMOJI: dict[str, tuple[str, str]] = {
    "ai_reply_icon": ("", "🤖"),   # иконка перед каждым ответом ИИ
    "hub_icon": ("", "🧭"),
    "support_icon": ("", "💛"),
}
...
def build_hub_keyboard(db: Database) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [mk_ikb(db, "btn_hub_model", "Выбрать модель", callback_data="hub_model")],
...
def build_model_keyboard(db: Database, current: str) -> InlineKeyboardMarkup:
    ...
    for key, label in AVAILABLE_MODELS:
        ...
        rows.append([InlineKeyboardButton(text=f"{mark}{label}", callback_data=f"setmodel:{key}")])
    return InlineKeyboardMarkup(inline_keyboard=rows)
...
def register_handlers(dp: Dispatcher, db: Database, settings: Settings) -> None:
    ...
    async def ai_reply(message: Message, model: str, prompt: str) -> None:
        ...
        system_prompt = DEV_AI_PERSONA + (f"\n\nДополнительно от создателя: {extra}" if extra else "")
        raw = await ai_chat(settings, model, system_prompt, prompt)
        if raw is None:
    ...
    async def handle_start(message: Message) -> None:
        ...
        await message.answer(
            "👋 Привет! Я <b>Dev AI</b> — меня создал @deverskyi на собственном железе. "
            "Пиши что угодно: код, идеи, помощь с проектом — отвечу по делу.\n\n"
            "Жми «🏠 Главное» внизу, чтобы увидеть все фишки: выбор модели, рисование, "
            "статьи в Telegraph, напоминания, опросы.",
    ...
    async def handle_support_cmd(message: Message) -> None:
        await message.answer(
            "💛 Если проект зашёл — можешь поддержать его развитие, или написать в "
            "поддержку, если что-то не работает.",
            reply_markup=build_support_keyboard(db),
        )
    ...
    # -- Hub: выбор модели ----------------------------------------------------
    @dp.callback_query(F.data == "hub_model")
    async def handle_hub_model(callback: CallbackQuery) -> None:
        current = db.get_user_model(callback.from_user.id, settings.default_model)
        await callback.message.answer("🧠 Выбери модель для ответов:", reply_markup=build_model_keyboard(db, current))
        await callback.answer()

    @dp.callback_query(F.data.startswith("setmodel:"))
    async def handle_set_model(callback: CallbackQuery) -> None:
        model = callback.data.split(":", 1)[1]
        if model not in MODEL_KEYS:
            await callback.answer("Такой модели нет.", show_alert=True)
            return
        db.set_user_model(callback.from_user.id, model)
        try:
    ...
    async def handle_hub_image(callback: CallbackQuery, state: FSMContext) -> None:
        ...
        await callback.message.answer("🎨 Опиши, что нарисовать:")
        await callback.answer()
    ...
    async def handle_image_prompt(message: Message, state: FSMContext) -> None:
        ...
        url = await generate_image(settings, image_model, message.text or "")
        if not url:
            await thinking.edit_text("⚠️ Не получилось сгенерировать изображение. Попробуй другой запрос.")
            return
        try:
            await message.answer_photo(url)
            await thinking.delete()
        except Exception:
            await thinking.edit_text(f"Готово: {url}")
    ...
    async def handle_hub_telegraph(callback: CallbackQuery, state: FSMContext) -> None:
        ...
        await callback.message.answer("📰 На какую тему написать статью?")
        await callback.answer()
    ...
    async def handle_telegraph_topic(message: Message, state: FSMContext) -> None:
        ...
        thinking = await message.answer("📰 Пишу статью...")
        model = db.get_user_model(message.from_user.id, settings.default_model)
        article = await ai_chat(
            settings, model,
        ...
        if not article:
            await thinking.edit_text("⚠️ Не получилось написать статью, попробуй ещё раз.")
            return
        url = await telegraph_create_page(db, topic[:80] or "Статья", article)
        if not url:
            await thinking.edit_text("⚠️ Не получилось опубликовать в Telegraph, попробуй позже.")
            return
        await thinking.edit_text(f"✅ Готово: {url}")
    ...
    async def handle_hub_reminder(callback: CallbackQuery, state: FSMContext) -> None:
        ...
        await callback.message.answer(
            "⏰ Напиши, о чём и когда напомнить. Понимаю форматы:\n"
    ...
    async def handle_hub_poll(callback: CallbackQuery, state: FSMContext) -> None:
        ...
        await callback.message.answer("📊 Напиши вопрос для опроса:")
        await callback.answer()
    ...
    async def handle_chat(message: Message, state: FSMContext) -> None:
        if await state.get_state() is not None:
            return  # сообщение обработает соответствующий FSM-хендлер выше
        db.upsert_user(message.from_user.id, message.from_user.first_name or "", message.from_user.username)
    ...
    def build_emoji_admin_keyboard() -> InlineKeyboardMarkup:
        ...
        for row in db.all_emoji_keys():
            ...
            rows.append([InlineKeyboardButton(text=f"{row['fallback'] or '⭐'} {key}", callback_data=f"admin_emoji_pick:{key}")])
    ...
    async def handle_emoji_forward(message: Message, state: FSMContext) -> None:
        ...
        await state.clear()
        await message.answer("✅ Эмодзи сохранён (обычный).", reply_markup=build_emoji_admin_keyboard())
    ...
    async def handle_button_icon_input(message: Message, state: FSMContext) -> None:
        ...
        await state.clear()
        await message.answer("✅ Иконка обновлена.", reply_markup=build_btnicons_keyboard())
