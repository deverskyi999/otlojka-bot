...
class Settings:
    ...
    cryptobot_api_key: str = ""  # можно также задать/сменить прямо из админ-панели
    ...
    def from_env(cls) -> "Settings":
        ...
        return cls(
            ...
            cryptobot_api_key=os.getenv("CRYPTOBOT_API_KEY", cls.cryptobot_api_key),
            cryptobot_asset=os.getenv("CRYPTOBOT_ASSET", cls.cryptobot_asset),
        )
...
DEFAULT_EMOJI: dict[str, tuple[str, str]] = {
    ...
    # у новых ключей emoji_id пуст, пока владелец не пришлёт реальный премиум-
    # эмодзи через админ-панель — тогда используется обычный юникод (fallback),
    # это безопаснее, чем хардкодить чужой/несуществующий emoji_id.
    "reminders_icon": ("", "⏰"),
    "profile_icon": ("", "👤"),
    "top_icon": ("", "🏆"),
    "terminal_icon": ("", "💻"),
...
TEXT_EMOJI_KEYS: list[tuple[str, str]] = [
    ...
    ("courses_icon", "Кнопка «Курсы»"),
    ("reminders_icon", "Кнопка «Напоминания»"),
    ("ai_icon", "Кнопка «ИИ-помощник»"),
    ("profile_icon", "Кнопка «Профиль»"),
    ("top_icon", "Кнопка «Топ»"),
    ("terminal_icon", "Кнопка «Терминал»"),
    ("buy_icon", "Кнопка оплаты Stars"),
    ("crypto_icon", "Кнопка оплаты CryptoBot"),
    ("promo_icon", "Кнопка промокода"),
    ("lock_icon", "Значок «заблокировано»"),
    ("check_icon", "Значок «пройдено»"),
    ("play_icon", "Значок «доступно»"),
    ("help_icon", "Иконка помощи /help"),
    ("platform_icon", "Кнопка «Как начать писать код»"),
    ("settings_icon", "Кнопка настроек в профиле"),
    ("star", "Общий значок ⭐ (топ/очки)"),
]
        ...
        payload = {
            ...
            "messages": [
                {"role": "system", "content": system_prompt or (
                    "Ты — ИИ-помощник по программированию внутри обучающего бота CodeSchool "
                    "(Python, JavaScript, C++). Отвечай кратко, понятно, с примерами кода через "
                    "Markdown (тройные обратные кавычки + язык), дружелюбно, без вступлений.\n\n"
                    "СТРОГО ПО ТЕМЕ: ты помогаешь ТОЛЬКО с программированием, кодом, отладкой, "
                    "уроками курса и вопросами про сам бот. Если вопрос не связан с программированием "
                    "или ботом (истории, сказки, страны, знаменитости, отвлечённые темы, личные советы "
                    "не по коду и т.п.) — НЕ отвечай по существу вопроса. Вместо этого ответь ровно так: "
                    "«Извините, я создан для того, чтобы помогать с программированием и учёбой в этом боте — "
                    "задайте вопрос по коду или по уроку 🙂»."
                )},
...
# ======================================================================
# CRYPTOBOT (Crypto Pay API) — оплата в USDT
...
async def cryptobot_create_invoice(
    db: "Database", settings: "Settings", user_id: int, amount: str, description: str
) -> Optional[dict]:
    """Создаёт инвойс в CryptoBot. Возвращает dict с полями invoice_id, pay_url
    (или mini_app_invoice_url) либо None при ошибке / отсутствии токена."""
    token = db.get_setting("cryptobot_token", "") or settings.cryptobot_api_key
    if not token:
        return None
    asset = db.get_setting("cryptobot_asset", settings.cryptobot_asset or "USDT")
    payload = {
        "asset": asset,
        "amount": amount,
        "description": description[:1000],
        "payload": f"sub_30d:{user_id}",
        "expires_in": 3600,
    }
    ...
    try:
        ...
        async with aiohttp.ClientSession(timeout=timeout, headers=headers) as session:
            async with session.post(f"{CRYPTOBOT_API_BASE}/createInvoice", json=payload) as resp:
                data = await resp.json()
                if not data.get("ok"):
...
async def cryptobot_get_invoice(db: "Database", settings: "Settings", invoice_id: str) -> Optional[dict]:
    token = db.get_setting("cryptobot_token", "") or settings.cryptobot_api_key
    if not token:
        return None
    headers = {"Crypto-Pay-API-Token": token, "Content-Type": "application/json"}
    try:
        ...
        async with aiohttp.ClientSession(timeout=timeout, headers=headers) as session:
            async with session.get(
                f"{CRYPTOBOT_API_BASE}/getInvoices", params={"invoice_ids": invoice_id}
            ) as resp:
                data = await resp.json()
                if not data.get("ok"):
                    return None
...
class Database:
    ...
    # -- crypto (CryptoBot) invoices ---------------------------------------
    def save_crypto_invoice(self, invoice_id: str, user_id: int) -> None:
        with self.connect() as conn:
            conn.execute(
                "INSERT OR IGNORE INTO crypto_invoices (invoice_id, user_id, status) VALUES (?, ?, 'active')",
                (invoice_id, user_id),
            )

    def mark_crypto_invoice(self, invoice_id: str, status: str) -> None:
        with self.connect() as conn:
            conn.execute("UPDATE crypto_invoices SET status = ? WHERE invoice_id = ?", (status, invoice_id))

    def get_crypto_invoice(self, invoice_id: str) -> Optional[sqlite3.Row]:
        with self.connect() as conn:
            return conn.execute(
                "SELECT * FROM crypto_invoices WHERE invoice_id = ?", (invoice_id,)
            ).fetchone()

    def pending_crypto_invoices(self) -> list[sqlite3.Row]:
        with self.connect() as conn:
            return conn.execute(
...
def btn_emoji(db: Database, key: str) -> str:
    """Юникод-эмодзи (fallback) для текста ОБЫЧНЫХ кнопок. Telegram Bot API
    не поддерживает премиум/кастомные эмодзи в inline/reply-кнопках — только
    в тексте сообщений, поэтому для кнопок используется только fallback,
    который тоже редактируется из админ-панели."""
    full = db.get_emoji_full(key)
    if full and full[1]:
        return full[1]
    return TEXT_EMOJI_FALLBACKS.get(key, "")
...
def build_main_reply_keyboard(db: Database) -> ReplyKeyboardMarkup:
    return ReplyKeyboardMarkup(
        keyboard=[
            [KeyboardButton(text=f"{btn_emoji(db, 'courses_icon')} Курсы"),
             KeyboardButton(text=f"{btn_emoji(db, 'reminders_icon')} Напоминания")],
            [KeyboardButton(text=f"{btn_emoji(db, 'ai_icon')} ИИ-помощник"),
             KeyboardButton(text=f"{btn_emoji(db, 'terminal_icon')} Терминал")],
            [KeyboardButton(text=f"{btn_emoji(db, 'profile_icon')} Профиль"),
             KeyboardButton(text=f"{btn_emoji(db, 'top_icon')} Топ")],
        ],
...
def build_language_keyboard(db: Database) -> InlineKeyboardMarkup:
    ...
    rows.append([InlineKeyboardButton(text=f"{btn_emoji(db, 'platform_icon')} Как начать писать код", callback_data="platform_guide")])
...
def build_course_list_keyboard(db: Database, settings: Settings, user_id: int, lang_key: str) -> InlineKeyboardMarkup:
    ...
    for course_key in COURSE_ORDER:
        ...
        if not course_is_unlocked(db, settings, user_id, lang_key, course_key):
            prefix = f"{btn_emoji(db, 'lock_icon')} "
        elif done >= total:
            prefix = f"{btn_emoji(db, 'check_icon')} "
        else:
            prefix = f"{btn_emoji(db, 'play_icon')} "
...
def build_buy_sub_keyboard(db: Database) -> InlineKeyboardMarkup:
    ...
    price_usdt = db.get_setting("price_usdt", "3")
    rows = [
        [InlineKeyboardButton(text=f"{btn_emoji(db, 'buy_icon')} Купить за {price} Stars", callback_data="pay_stars")],
    ]
    rows.append([InlineKeyboardButton(
        text=f"{btn_emoji(db, 'crypto_icon')} Оплатить {price_usdt} USDT (CryptoBot)", callback_data="pay_crypto"
    )])
    rows.append([InlineKeyboardButton(text=f"{btn_emoji(db, 'promo_icon')} У меня есть промокод", callback_data="activate_promo_btn")])
    return InlineKeyboardMarkup(inline_keyboard=rows)
...
def build_admin_menu_keyboard(db: Database) -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup(
        inline_keyboard=[
            ...
            [InlineKeyboardButton(text="💠 CryptoBot API-ключ", callback_data="admin_crypto_key")],
...
def build_emoji_admin_keyboard(db: Database) -> InlineKeyboardMarkup:
    rows = []
    for key, label in TEXT_EMOJI_KEYS:
        current = btn_emoji(db, key)
...
def register_handlers(dp: Dispatcher, db: Database, settings: Settings) -> None:
    ...
    async def show_lesson(bot: Bot, chat_id: int, lang_key: str, course_key: str, idx: int, user_id: int) -> None:
        ...
        if is_new and not db.try_consume_daily_lesson(user_id):
    ...
    @dp.message(F.text.endswith("Курсы"))
    ...
    async def handle_help_cmd(message: Message) -> None:
        await message.answer(
            ...
            "🤖 <b>ИИ-помощник</b> — вопросы по коду и по урокам (строго по теме программирования)\n"
            "💻 <b>Терминал</b> — выбери язык, пришли код, ИИ покажет вывод и объяснит его\n"
    ...
    @dp.message(F.text.endswith("Топ"))
    ...
    # -- Мини-терминал (без WebApp): выбор языка кнопкой -> код -> ИИ «выполняет» --
    @dp.message(F.text.endswith("Терминал"))
    async def handle_terminal_btn(message: Message) -> None:
        ...
        icon = btn_emoji(db, "terminal_icon")
        await message.answer(f"{icon} Выбери язык для мини-терминала:", reply_markup=kb)

    @dp.callback_query(F.data.startswith("term_lang:"))
    async def handle_terminal_lang(callback: CallbackQuery, state: FSMContext) -> None:
        lang = callback.data.split(":", 1)[1]
        await state.set_state(UserStates.waiting_terminal_code)
        await state.update_data(term_lang=lang)
        names = {"python": "Python", "javascript": "JavaScript", "cpp": "C++"}
        await callback.message.answer(
            f"Пришли код на {names.get(lang, lang)} одним сообщением — покажу, что выведет терминал, "
            "и коротко объясню результат."
        )
        await callback.answer()
    ...
    async def handle_terminal_code(message: Message, state: FSMContext) -> None:
        ...
        names = {"python": "Python", "javascript": "JavaScript", "cpp": "C++"}
        icon = btn_emoji(db, "terminal_icon")
        thinking = await message.answer(f"{icon} Выполняю в уме...")
        system_prompt = (
            f"Ты симулируешь терминал/консоль для языка {names.get(lang, lang)}. "
            "Тебе дают код ученика. Определи, что именно выведет программа при запуске "
            "(stdout построчно), включая ошибки компиляции/выполнения, если код некорректен. "
            "Формат ответа СТРОГО такой:\n"
            "1) блок кода с пометкой 'output' — только то, что напечатает терминал (без пояснений внутри блока);\n"
            "2) после блока — 2-4 коротких предложения на русском, объясняющих, почему получился такой результат.\n"
            "Если в коде есть очевидная ошибка — покажи именно то сообщение об ошибке, которое выдал бы "
            "настоящий интерпретатор/компилятор."
        )
        answer = await _ai_chat_completion(
            settings, f"Код:\n```{lang}\n{code}\n```", system_prompt=system_prompt
        )
        body = _markdown_to_telegram_html(answer) if answer else "⚠️ Не получилось выполнить, попробуй ещё раз."
        try:
            await thinking.edit_text(f"{icon} <b>Терминал ({names.get(lang, lang)}):</b>\n{body}", parse_mode="HTML")
        except Exception:
            await message.answer(f"{icon} <b>Терминал ({names.get(lang, lang)}):</b>\n{body}", parse_mode="HTML")

    @dp.message(F.text.endswith("ИИ-помощник"))
    ...
    async def handle_profile_btn(message: Message) -> None:
        ...
        await message.answer(
            ...
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                [InlineKeyboardButton(text="⚙️ Уроков в день", callback_data="daily_limit_menu")],
            ]),
        )
    ...
    # -- Напоминания ---------------------------------------------------
    @dp.message(F.text.endswith("Напоминания"))
    ...
    async def handle_pay_crypto(callback: CallbackQuery) -> None:
        ...
        invoice = await cryptobot_create_invoice(
            db, settings, callback.from_user.id, price_usdt,
            "Подписка CodeSchool на 30 дней",
        )
        if not invoice:
            await callback.message.answer(
                "⚠️ Оплата через CryptoBot сейчас недоступна (не настроен API-ключ). "
                "Владелец может добавить его в админ-панели → «CryptoBot API-ключ»."
            )
            await callback.answer()
            return
        db.save_crypto_invoice(str(invoice["invoice_id"]), callback.from_user.id)
        ...
        kb = InlineKeyboardMarkup(inline_keyboard=[
            ...
            [InlineKeyboardButton(text="🔄 Я оплатил, проверить", callback_data=f"check_crypto:{invoice['invoice_id']}")],
        ])
        await callback.message.answer(
            f"Счёт на {price_usdt} USDT создан. Оплати по кнопке ниже — подписка активируется "
            "автоматически в течение ~20 секунд после оплаты, либо нажми «Проверить».",
            reply_markup=kb,
        )
        await callback.answer()
    ...
    async def handle_check_crypto(callback: CallbackQuery) -> None:
        ...
        result = await cryptobot_get_invoice(db, settings, invoice_id)
        if not result:
            await callback.answer("Не удалось проверить счёт, попробуй позже.", show_alert=True)
            return
        status = result.get("status")
        if status == "paid":
            local = db.get_crypto_invoice(invoice_id)
            if local and local["status"] != "paid":
                db.mark_crypto_invoice(invoice_id, "paid")
                new_until = db.grant_subscription(callback.from_user.id, 30)
                until_str = datetime.fromtimestamp(new_until).strftime("%d.%m.%Y")
                await callback.message.answer(
                    f"✅ Оплата подтверждена! Подписка активна до <b>{until_str}</b>.", parse_mode="HTML"
                )
            await callback.answer("Оплачено ✅")
        elif status == "expired":
            db.mark_crypto_invoice(invoice_id, "expired")
    ...
    async def handle_admin_emoji(callback: CallbackQuery) -> None:
        if not _is_owner(callback.from_user.id, settings):
            ...
            return
        await callback.message.edit_text(
            "💎 Выбери, для какого текста/кнопки задать эмодзи:",
    ...
    async def handle_admin_emoji_pick(callback: CallbackQuery, state: FSMContext) -> None:
        ...
        await callback.message.answer(
            "Пришли премиум-эмодзи (как сообщение с одним эмодзи) — он покажется в тексте сообщений.\n"
            "Или пришли обычный юникод-эмодзи текстом (например 🚀) — это будет использовано и в тексте, и на кнопках "
            "(на кнопках Telegram премиум-эмодзи не показывает, только обычный юникод)."
        )
        await callback.answer()
    ...
    async def handle_emoji_forward(message: Message, state: FSMContext) -> None:
        ...
        if custom:
            ...
            # обычный юникод-эмодзи/символ без премиум-статуса — сохраняем только как fallback
            db.set_emoji(key, "", message.text.strip())
        else:
            await message.answer("Не нашёл ни премиум-эмодзи, ни текста. Пришли ещё раз.")
            return
        await state.clear()
        await message.answer("✅ Эмодзи сохранён.", reply_markup=build_emoji_admin_keyboard(db))

    @dp.callback_query(F.data == "admin_crypto_key")
    async def handle_admin_crypto_key(callback: CallbackQuery, state: FSMContext) -> None:
        if not _is_owner(callback.from_user.id, settings):
            await callback.answer()
            return
        current = db.get_setting("cryptobot_token", "")
        masked = (current[:4] + "…" + current[-4:]) if len(current) > 8 else ("не задан" if not current else current)
        await state.set_state(AdminStates.waiting_crypto_key)
        await callback.message.answer(
            f"Текущий ключ CryptoBot: <code>{escape(masked)}</code>\n\n"
            "Пришли новый API-токен из @CryptoBot → Crypto Pay → Create App.\n"
            "Пришли «-» чтобы очистить ключ (отключить оплату CryptoBot).",
            parse_mode="HTML",
        )
        await callback.answer()

    @dp.message(AdminStates.waiting_crypto_key)
    async def handle_crypto_key_input(message: Message, state: FSMContext) -> None:
        await state.clear()
        text = (message.text or "").strip()
        db.set_setting("cryptobot_token", "" if text == "-" else text)
        await message.answer("✅ Ключ CryptoBot сохранён.", reply_markup=build_admin_menu_keyboard(db))
    ...
    async def handle_admin_price(callback: CallbackQuery, state: FSMContext) -> None:
        if not _is_owner(callback.from_user.id, settings):
            await callback.answer()
            return
        stars = db.get_setting("price_stars", "150")
        usdt = db.get_setting("price_usdt", "3")
        await state.set_state(AdminStates.waiting_price_stars)
        await callback.message.answer(
            f"Текущая цена: {stars} Stars / {usdt} USDT.\nПришли новую цену в Stars (число):"
        )
        await callback.answer()

    @dp.message(AdminStates.waiting_price_stars)
    async def handle_price_stars_input(message: Message, state: FSMContext) -> None:
        if not (message.text or "").strip().isdigit():
            await message.answer("Нужно число. Попробуй ещё раз:")
            return
        db.set_setting("price_stars", message.text.strip())
        await state.set_state(AdminStates.waiting_price_usdt)
        await message.answer("Принято. Теперь пришли цену в USDT (число, например 3):")

    @dp.message(AdminStates.waiting_price_usdt)
    async def handle_price_usdt_input(message: Message, state: FSMContext) -> None:
        try:
            float((message.text or "").strip().replace(",", "."))
        except ValueError:
            await message.answer("Нужно число. Попробуй ещё раз:")
            return
        db.set_setting("price_usdt", message.text.strip())
        await state.clear()
        await message.answer("✅ Цены обновлены.", reply_markup=build_admin_menu_keyboard(db))
...
async def crypto_invoice_loop(bot: Bot, db: Database, settings: Settings) -> None:
    """Раз в 20 секунд проверяет активные CryptoBot-инвойсы; при оплате сразу
    продлевает подписку и уведомляет пользователя — своего рода 'автопродление'
    в рамках возможностей Crypto Pay API (полноценных рекуррентных списаний
    CryptoBot не поддерживает, поэтому это оплата по инвойсу + мгновенная
    активация без участия пользователя после факта оплаты)."""
    while True:
        try:
            for inv in db.pending_crypto_invoices():
                result = await cryptobot_get_invoice(db, settings, inv["invoice_id"])
                if not result:
                    continue
                status = result.get("status")
                if status == "paid":
                    db.mark_crypto_invoice(inv["invoice_id"], "paid")
                    new_until = db.grant_subscription(inv["user_id"], 30)
                    until_str = datetime.fromtimestamp(new_until).strftime("%d.%m.%Y")
                    try:
                        await bot.send_message(
                            inv["user_id"],
                            f"✅ Оплата через CryptoBot получена! Подписка активна до <b>{until_str}</b>.",
                            parse_mode="HTML",
                        )
                    except Exception:
                        logger.exception("Failed to notify user_id=%s about crypto payment", inv["user_id"])
                elif status == "expired":
                    db.mark_crypto_invoice(inv["invoice_id"], "expired")
        except Exception:
            logger.exception("Crypto invoice loop iteration failed")
...
async def main() -> None:
    ...
    asyncio.create_task(crypto_invoice_loop(bot, db, settings))
